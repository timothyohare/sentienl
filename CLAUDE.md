...
**`sentinel-price-followup.service` was blind to ASX for its entire life, same bug class as the Aug-23 stale-config incident above — found and fixed 2026-08-26**: the service process had been running continuously since 2026-08-23T20:09 AEST, but the commit adding `AsxPriceFetcher` (59d8a56) landed an hour later, 21:12 AEST. The live process's `CompositePriceFetcher` simply never had an ASX fetcher in its list, so every backfill attempt for any of the 74 ASX HIGH signals logged since market open (10:00 AEST) silently returned "No price available" with **no underlying IB or yfinance log line at all** — the giveaway that the fetcher wasn't being called, vs. the fetcher being called and failing. Restarting the service fixed it immediately: the next pass issued real `reqHistoricalData` calls for all ten tickers and backfilled t15/t60 for all 74 pending signals in one shot. `signal_scorecard.py` now shows `asx volume_spike` beating baseline at t15/t60/t240 (t1440 still cold-start `[proxy]`). **Lesson reinforced**: any commit that changes `price_followup.py`'s fetcher logic needs `sentinel-price-followup.service` restarted, not just the collector's own service — the "restart every service you touch" rule doesn't obviously extend to price-followup because it's not the service you edited code *for*, but it imports that code too.
